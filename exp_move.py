...
def train(train_loader, args):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            if args.isRestore:
                inputs = copy.deepcopy(restoreModel(inputs).cpu().detach().numpy())
        ...
        if val_loss < best_loss:
            ...
            torch.save(model, './' + args.data + "_move_"+'latest_model.pt')
            best_loss = val_loss
            patient = 0
        else:
            patient += 1
...
def test(model, data_loader):
    model = torch.load('./' + args.data + "_move_" + 'latest_model.pt')
    model.to(device)
